...
def expand_mention_tokens(markdown: str) -> str:
    """Replaces mention tokens in a Markdown string with `marklas`'s `Mention` spans.

    Tokens that do not match :data:`MENTION_TOKEN_PATTERN` are left untouched and will be submitted as literal text.

    Args:
        markdown: the comment text in Markdown, possibly containing ``@[Name](accountId)`` tokens.

    Returns:
        The Markdown with every recognized mention token expanded to a ``marklas`` mention span. If the input
